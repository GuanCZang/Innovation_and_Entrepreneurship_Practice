...
typedef struct {
	unsigned int state[8];	// ABCDEFGH状态
	unsigned int length;	// 总消息长度
	unsigned int curlen;	// 当前消息块长度
	unsigned char buf[64];	// 保存输入的消息及其填充
} SM3_STATE;

typedef struct {			//定义append的结构体
	unsigned int state[8];	// ABCDEFGH状态
	unsigned int length;	// 总消息长度
	unsigned int curlen;	// 当前消息块长度
	unsigned char buf[64];	// 保存输入的消息及其填充
...
void SM3_Append_init(SM3_APPEND_STATE* mdA);
...
void SM3_Append_256(unsigned char buf[], int len, unsigned char hash[]);
//int SM3_SelfTest();
